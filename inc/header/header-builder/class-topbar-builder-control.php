...
if (class_exists('WP_Customize_Control')) {
    class HT_Topbar_Builder_Control extends WP_Customize_Control
    {
        ...
        public function render_content()
        {
            ...
            echo '<ul id="ht-topbar-sortable" class="ht-topbar-sortable flex flex-row flex-wrap gap-3 p-2 border border-gray-300 rounded-md bg-white min-h-[50px]">';
            foreach ($value as $item) {
                echo $this->render_sortable_item($item);
            }
            echo '</ul>';
            echo '<p class="text-sm font-medium mt-4 mb-1">Thành phần còn lại</p>';
            echo '<ul id="ht-topbar-available" class="ht-topbar-sortable flex flex-row flex-wrap gap-3 p-2 border border-dashed border-gray-300 rounded-md bg-gray-50 min-h-[50px]">';
